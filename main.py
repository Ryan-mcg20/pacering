...
class MainMonitorScreen(Screen):
    ...
    def handle_profile_tap(self):
        ...
        if self.tap_count == 1:
            # Single tap - wait a tiny bit to see if it's a multi-tap. 
            # In Kivy, simpler to just route single taps to settings if it doesn't interrupt dev mode,
            # but to allow dev mode, we schedule settings open slightly delayed.
            Clock.schedule_once(self._go_to_settings, 0.4)
            
        if self.tap_count == 5:
            Clock.unschedule(self._go_to_settings) # Cancel settings navigation
            self.open_dev_mode()
            self.tap_count = 0
    ...
    def update_monitor(self, dt):
        ...
        # --- SIMULATOR LOGIC ---
        # Note: If USE_SIMULATOR is False, replace this block with calls to ble_manager.py
        if USE_SIMULATOR:
            self.sim_tick += 1
            if self.simulator_scenario == 'resting':
                ...
                target = resting + ((self.current_bpm - resting) * 0.9) # Exponential decay
            ...
            # INTEGRATION POINT: Fetch from ble_manager.py
            # e.g., self.current_bpm = app.ble_manager.get_current_bpm()
            pass
        ...
        target_color = '#ede9fe' # Rest
        if self.current_bpm >= threshold:
            target_color = '#f43f5e' # Danger
        elif self.current_bpm >= resting + 15:
            target_color = '#e879f9' # Elevated
        ...
        # --- ALERT ENGINE LOGIC ---
        # Note: If using alert_engine.py, call app.alert_engine.process(self.current_bpm) here
...
class PaceRingApp(App):
    ...
    def build(self):
        # Dynamically inject smooth button class into Kivy factory to use in Dev mode
    ...
    def setup_main_screen(self):
        # Refresh main screen logic if needed upon finishing onboarding
...
# Helper function needed for Kivy dp() conversions in KV string context if done via python
def dp(val):
